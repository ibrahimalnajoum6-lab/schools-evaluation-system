...
<div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; background: #fff; padding: 10px 14px; border-radius: 12px; box-shadow: 0 2px 6px rgba(0,0,0,0.04);'>
    <div>
        <span style='font-size: 16px; font-weight: 800; color: #0d5c3a;'>👤 {st.session_state.user['name']}</span><br>
        <span style='font-size: 12px; color: #64748b;'>{st.session_state.user['specialization']} | {st.session_state.user['role']}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
# -------------------------------------------------------------
# 1. شاشة استمارة تقييم جديدة (تنسيق مخصص للهواتف)
# -------------------------------------------------------------
if choice == "📝 تقييم جديد":
    
    # تهيئة الدرجات في Session State للتحديث الفوري
    for item in CRITERIA:
        k = f"q_{item['id']}"
    ...
    # تبويبات المجالات لتقليل التمرير على الهاتف
    ...
    for idx, domain in enumerate(DOMAINS):
        with tabs[idx]:
            ...
            for item in domain_items:
                ...
                # خيارات لمس سريعة ومريحة للأصابع
...
# -------------------------------------------------------------
# 2. سجل الزيارات بتصميم مريح للجوال
# -------------------------------------------------------------
